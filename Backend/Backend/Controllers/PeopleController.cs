...
public class PeopleController : ControllerBase
{
    ...
    public async Task<IActionResult> AddPerson(Person person)
    {
        try
        {
            ...
            return CreatedAtRoute("GetPerson", new { id = person.Id }, person); // 201 Created status code + location of the resource (http://localhost:5000/api/people/{id}) + person object in the response body
        }
        catch (Exception ex)
        {
